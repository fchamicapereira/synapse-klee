...
ExecutionState::ExecutionState(const ExecutionState &state)
    : fnAliases(state.fnAliases), readsIntercepts(state.readsIntercepts),
      ...
      havocs(state.havocs), noHavocs(state.noHavocs),
      havocNames(state.havocNames), noHavocNames(state.noHavocNames),
...
void ExecutionState::addHavocInfo(const MemoryObject *mo,
  ...
  if (!loopInProcess.isNull()) {
    klee_error("You must call klee_possibly_havoc(%s) outside of a "
               "loop subject to invariant analysis.",
               name.c_str());
  }
  havocs[mo].name = name;
...
void ExecutionState::dumpStack() const { dumpStack(llvm::errs()); }
...
void ExecutionState::traceArgArr(ref<Expr> arg, Expr::Width width, size_t count,
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    traceArgPtrField(arg, i * width / 8, width, std::to_string(i),
                     tracePointeeIn, tracePointeeOut);
  }
}
...
void ExecutionState::traceArgPtrFieldArr(ref<Expr> arg, int offset,
                                         Expr::Width el_width, int count,
                                         std::string name, bool doTraceValueIn,
                                         bool doTraceValueOut) {
  if (count == 0) {
    return traceArgPtrField(arg, offset, el_width, name, doTraceValueIn,
                            doTraceValueOut);
  }
  assert(!callPath.empty() && callPath.back().f == stack.back().kf->function &&
         "Must trace the function first to trace a particular field.");
  ...
  for (int i = 0; i < count; ++i) {
    // width is given in bits, we need bytes for the offset
    traceArgPtrNestedField(arg, offset, i * el_width / 8, el_width,
                           std::to_string(i), doTraceValueIn, doTraceValueOut);
  }
}

void ExecutionState::traceArgPtrNestedNestedField(
    ref<Expr> arg, int base_base_offset, int base_offset, int offset,
  ...
  assert(argInfo->pointee.fields[base_base_offset].fields.count(base_offset) !=
             0 &&
         "Must first trace the base field iself.");
  assert(argInfo->pointee.fields[base_base_offset]
                 .fields[base_offset]
  ...
  size_t base = (cast<ConstantExpr>(arg))->getZExtValue();
  if (trace_in) {
    ref<ConstantExpr> addrExpr = ConstantExpr::alloc(
        base + base_offset + base_base_offset + offset, sizeof(size_t) * 8);
    descr.inVal = readMemoryChunk(addrExpr, width, true);
  }
  descr.addr = base + base_offset + offset;
  descr.doTraceValueIn = trace_in;
  descr.doTraceValueOut = trace_out;
  argInfo->pointee.fields[base_base_offset].fields[base_offset].fields[offset] =
      descr;
}
...
void ExecutionState::traceArgPtrNestedFieldArr(ref<Expr> arg, int base_offset,
                                               int offset, Expr::Width width,
                                               int count, std::string name,
                                               bool trace_in, bool trace_out) {
  if (count == 0) {
    return traceArgPtrNestedField(arg, base_offset, offset, width, name,
                                  trace_in, trace_out);
  }
  assert(!callPath.empty() && callPath.back().f == stack.back().kf->function &&
         "Must trace the function first to trace a particular field.");
  ...
  for (int i = 0; i < count; ++i) {
    // width is given in bits, we need bytes for the offset
    traceArgPtrNestedNestedField(arg, base_offset, offset, i * width / 8, width,
                                 std::to_string(i), trace_in, trace_out);
  }
}
...
void ExecutionState::traceExtraPtrNestedFieldArr(size_t ptr, int base_offset,
                                                 int offset, Expr::Width width,
                                                 int count, std::string name,
                                                 bool trace_in,
                                                 bool trace_out) {
  if (count == 0) {
    return traceExtraPtrNestedField(ptr, base_offset, offset, width, name,
                                    trace_in, trace_out);
  }
  assert(!callPath.empty() && callPath.back().f == stack.back().kf->function &&
         "Must trace the function first to trace a particular field.");
  ...
  for (int i = 0; i < count; ++i) {
    // width is given in bits, we need bytes for the offset
    traceExtraPtrNestedNestedField(ptr, base_offset, offset, i * width / 8,
                                   width, std::to_string(i), trace_in,
                                   trace_out);
  }
}
...
void ExecutionState::traceExtraPtrArr(size_t ptr, Expr::Width width,
                                      size_t count, std::string name,
                                      std::string type, bool trace_in,
                                      bool trace_out) {
  if (count == 0) {
  ...
  if (trace_in) {
    extraPtr->pointee.inVal = constraints.simplifyExpr(readMemoryChunk(
        ConstantExpr::alloc(ptr, sizeof(size_t) * 8), width * count, true));
    indirectSymbols = GetExprSymbols::visit(extraPtr->pointee.inVal);
  }
  std::vector<ref<Expr>> constrs = relevantConstraints(indirectSymbols);
  callPath.back().callContext.insert(callPath.back().callContext.end(),
  ...
  for (size_t i = 0; i < count; ++i) {
    // width is given in bits, we need bytes for the offset
    traceExtraPtrField(ptr, i * width / 8, width, std::to_string(i), trace_in,
                       trace_out);
  }
}
...
void ExecutionState::traceExtraPtrFieldArray(size_t ptr, int offset,
                                             Expr::Width el_width, int count,
                                             std::string name, bool trace_in,
                                             bool trace_out) {
  if (count == 0) {
  ...
  for (int i = 0; i < count; ++i) {
    // width is given in bits, we need bytes for the offset
    traceExtraPtrNestedField(ptr, offset, i * el_width / 8, el_width,
                             std::to_string(i), trace_in, trace_out);
  }
}
      ...
      if (0 != refVal->compare(*val)) {
        ...
        val->dump();
          ...
          if (state.noHavocs.find(obj) != state.noHavocs.end()) {
            ...
            klee_error("Guaranteed invariant (never-havoc %s) changed during "
                       "invariant analysis:\n"
                       "  name: %s\n  location: %s\n"
                       "  local: %s\n  global: %s\n"
                       "  fixed: %s\n  size: %u\n"
