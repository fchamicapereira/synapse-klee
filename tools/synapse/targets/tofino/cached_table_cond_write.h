...
public:
  CachedTableConditionalWrite(const bdd::Node *node, DS_ID _id, addr_t _obj,
  ...
  virtual Module *clone() const override {
    Module *cloned = new CachedTableConditionalWrite(
        node, id, obj, key, read_value, write_value, map_has_this_key,
        cache_update_failed);
    return cloned;
  }
...
protected:
  ...
  process_node(const EP *ep, const bdd::Node *node) const override {
    ...
    symbol_t cache_update_failed = create_cache_update_failed_symbol();

    Module *module = new CachedTableConditionalWrite(
        node, cached_table->id, data.obj, data.key, data.read_value,
        data.write_value, data.map_has_this_key, cache_update_failed);
    EPNode *cached_table_cond_write_node = new EPNode(module);

    EP *new_ep = new EP(*ep);
    new_eps.push_back(new_ep);

    klee::ref<klee::Expr> cache_update_success_condition =
        build_cache_update_success_condition(cache_update_failed);

    bdd::Node *on_cache_update_success;
    bdd::Node *on_cache_update_failed;
    bdd::BDD *new_bdd = branch_bdd_on_cache_update_success(
        new_ep, node, data.obj, cache_update_success_condition, coalescing_data,
        new_bdd, on_cache_update_success, on_cache_update_failed);

    symbols_t symbols = get_dataplane_state(ep, node);

    Module *if_module = new If(node, {cache_update_success_condition});
    Module *then_module = new Then(node);
    Module *else_module = new Else(node);
    Module *send_to_controller_module =
        new SendToController(on_cache_update_failed, symbols);
    ...
    EPLeaf on_cache_update_success_leaf(then_node, on_cache_update_success);
    EPLeaf on_cache_update_failed_leaf(send_to_controller_node,
                                       on_cache_update_failed);

    new_ep->process_leaf(
        cached_table_cond_write_node,
        {on_cache_update_success_leaf, on_cache_update_failed_leaf});
...
private:
  ...
  klee::ref<klee::Expr> build_cache_update_success_condition(
      const symbol_t &cache_update_failed) const {
    klee::ref<klee::Expr> zero = kutil::solver_toolbox.exprBuilder->Constant(
        0, cache_update_failed.expr->getWidth());
    return kutil::solver_toolbox.exprBuilder->Eq(cache_update_failed.expr,
                                                 zero);
  }

  symbol_t create_cache_update_failed_symbol() const {
    const klee::Array *array;
    std::string label = "cache_update_failed";
    bits_t size = 32;
  ...
  void delete_dchain_ops_on_cache_update_success(
      const EP *ep, bdd::BDD *bdd, addr_t map_obj,
      const map_coalescing_data_t &coalescing_data,
      bdd::Node *on_cache_update_success) const {
    std::vector<const bdd::Node *> dchain_ops_to_remove =
        get_all_functions_after_node(on_cache_update_success,
                                     {
    ...
    for (const bdd::Node *dchain_op : dchain_ops_to_remove) {
      ...
      if (call.function_name == "dchain_allocate_new_index") {
        ...
        const bdd::Branch *index_alloc_check = find_branch_checking_index_alloc(
            ep, on_cache_update_success, out_of_space);
  ...
  void delete_map_puts_on_cache_update_success(
      const EP *ep, bdd::BDD *bdd, addr_t target_map,
      bdd::Node *on_cache_update_success) const {
    std::vector<const bdd::Node *> map_puts_to_remove =
        get_all_functions_after_node(on_cache_update_success, {"map_put"});
  ...
  void delete_vector_key_ops_on_cache_update_success(
      const EP *ep, bdd::BDD *bdd, addr_t target_map,
      const map_coalescing_data_t &coalescing_data,
      bdd::Node *on_cache_update_success) const {
    std::vector<const bdd::Node *> vector_ops = get_all_functions_after_node(
        on_cache_update_success, {"vector_borrow", "vector_return"});
  ...
  void add_map_get_clone_on_cache_update_failed(
      const EP *ep, bdd::BDD *bdd, const bdd::Node *map_get,
      const bdd::Branch *cache_update_branch,
      bdd::Node *&new_on_cache_update_failed) const {
    bdd::node_id_t &id = bdd->get_mutable_id();
    bdd::NodeManager &manager = bdd->get_mutable_manager();
    bdd::Node *map_get_on_cache_update_failed = map_get->clone(manager, false);
    map_get_on_cache_update_failed->recursive_update_ids(id);

    add_non_branch_nodes_to_bdd(ep, bdd, cache_update_branch->get_on_false(),
                                {map_get_on_cache_update_failed},
                                new_on_cache_update_failed);
  }

  void replicate_hdr_parsing_ops_on_cache_update_failed(
      const EP *ep, bdd::BDD *bdd, const bdd::Branch *cache_update_branch,
      bdd::Node *&new_on_cache_update_failed) const {
    const bdd::Node *on_cache_update_failed =
        cache_update_branch->get_on_false();

    std::vector<const bdd::Node *> prev_borrows = get_prev_functions(
        ep, on_cache_update_failed, {"packet_borrow_next_chunk"});

    if (prev_borrows.empty()) {
      return;
    }

    add_non_branch_nodes_to_bdd(ep, bdd, on_cache_update_failed, prev_borrows,
                                new_on_cache_update_failed);
  }

  bdd::BDD *branch_bdd_on_cache_update_success(
      const EP *ep, const bdd::Node *map_get, addr_t map_obj,
      klee::ref<klee::Expr> cache_update_success_condition,
      const map_coalescing_data_t &coalescing_data, bdd::BDD *&new_bdd,
      bdd::Node *&on_cache_update_success,
      bdd::Node *&on_cache_update_failed) const {
    ...
    bdd::Branch *cache_update_branch;
    add_branch_to_bdd(ep, new_bdd, next, cache_update_success_condition,
                      cache_update_branch);
    on_cache_update_success = cache_update_branch->get_mutable_on_true();

    add_map_get_clone_on_cache_update_failed(
        ep, new_bdd, map_get, cache_update_branch, on_cache_update_failed);
    replicate_hdr_parsing_ops_on_cache_update_failed(
        ep, new_bdd, cache_update_branch, on_cache_update_failed);

    delete_map_puts_on_cache_update_success(ep, new_bdd, map_obj,
                                            on_cache_update_success);
    delete_dchain_ops_on_cache_update_success(
        ep, new_bdd, map_obj, coalescing_data, on_cache_update_success);
    delete_vector_key_ops_on_cache_update_success(
        ep, new_bdd, map_obj, coalescing_data, on_cache_update_success);
