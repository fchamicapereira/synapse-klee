...
#define KEY_EQ(PREFIX, KEY_SIZE_BYTES)                                         \
  bool PREFIX##_key_eq(void *obj1, void *obj2) {                               \
...
#define KEY_HASH(PREFIX, KEY_SIZE_BYTES)                                       \
  unsigned PREFIX##_key_hash(void *obj) {                                      \
    uint8_t *bytes = (uint8_t *)obj;                                           \
    unsigned hash = 0;                                                         \
    for (int i = 0; i < (KEY_SIZE_BYTES); i++)                                 \
      hash = (unsigned int)(__builtin_ia32_crc32di(                            \
          hash, (unsigned long long)(bytes[i] & 0xff) & 0xffffffff));          \
    return hash;                                                               \
  }
...
#define INIT_ELEM(PREFIX, ELEM_SIZE_BYTES)                                     \
  void PREFIX##_init_elem(void *elem) {                                        \
    uint8_t *b = (uint8_t *)elem;                                              \
    for (int i = 0; i < (ELEM_SIZE_BYTES); i++)                                \
      b[i] = 0;                                                                \
  }
...
int main(int argc, char **argv) {
  ...
  for (uint16_t device = 0; device < nb_devices; device++) {
    ...
    if (ret == 0) {
      printf("Initialized device %" PRIu16 ".", device);
    } else {
